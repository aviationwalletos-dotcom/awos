...
export async function buildPilotFlightExperienceCertificatePdf(entries: LogbookEntry[], holder: PilotCertificateHolder = {}): Promise<Uint8Array> {
  ...
  const notes = [
    ...
    '주간·야간 × 시계·야외 구분은 로그북의 주간/야간 시간과 야외비행 시간으로 배정한 값이며(야간 야외가 없는 기록은 야간·야외 중 작은 값), "기장/기장 외" 열은 해당 비행의 PIC 시간 유무로, 학생조종사 열은 PIC 없이 교육받은 비행으로 나눴습니다. 모의비행 열은 모의계기 시간, 기타 열은 모의비행훈련장치 시간입니다. 미인증 비행경력증명서 이월 기록은 제외했습니다.',
