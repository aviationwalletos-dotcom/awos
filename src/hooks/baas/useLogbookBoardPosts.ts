...
interface UseLogbookBoardPostsReturn {
...
async function fetchLogbookPostsPage(offset: number, limit: number): Promise<BoardPostListResponse> {
  ...
  return withRetry(async () => {
    ...
    if (!response.ok && isRetryableStatus(response.status)) {
      throw new RetryableHttpError(response.status, `비행기록 게시글 목록을 불러오지 못했습니다. (상태 코드: ${response.status})`)
    }

    const result = await parseJsonResponse<ApiEnvelope<BoardPostListResponse>>(response)

    if (result.result !== 'SUCCESS' || !result.data) {
      throw new Error(result.message || '비행기록 게시글 목록을 불러오지 못했습니다.')
    }
...
export function useLogbookBoardPosts(options: UseLogbookBoardPostsOptions = {}): UseLogbookBoardPostsReturn {
  ...
  const fetchPosts = useCallback(async (): Promise<BoardPostListResponse | null> => {
    ...
    try {
      const pageData = await fetchLogbookPostsPage(0, limitRef.current)
      setData(pageData)
      return pageData
    } catch (err) {
      const message = err instanceof Error ? err.message : '비행기록 게시글 목록을 불러오지 못했습니다.'
      setError(message)
      return null
    } finally {
      setIsLoading(false)
  ...
  const refetchAll = useCallback(async (): Promise<BoardPostListItem[] | null> => {
    ...
    try {
      ...
      for (let page = 0; page < MAX_PAGES && offset < totalCount; page += 1) {
        try {
          const pageData = await fetchLogbookPostsPage(offset, PAGE_SIZE)
