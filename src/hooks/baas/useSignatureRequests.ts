...
export function useSignatureRequests(options: UseSignatureRequestsOptions = {}): UseSignatureRequestsReturn {
  ...
  const fetchRequests = useCallback(async (): Promise<BoardPostListResponse | null> => {
    setIsLoading(true)
    setError(null)
    ...
    try {
      ...
      const response = await baasFetch(
        `${BAAS_BASE_URL}/public/boards/${getBaasProjectId()}/${SIGNATURE_REQUEST_BOARD_ID}/posts?${params.toString()}`,
        {
          method: 'GET',
      ...
      if (result.result !== 'SUCCESS') {
        throw new Error(result.message || '서명 요청 목록을 불러오지 못했습니다.')
      }

      setData(result.data ?? null)
      return result.data ?? null
    } catch (err) {
      const message = err instanceof Error ? err.message : '서명 요청 목록을 불러오지 못했습니다.'
      setError(message)
      return null
    } finally {
      setIsLoading(false)
