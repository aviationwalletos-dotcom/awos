...
interface UseInquiryBoardPostsOptions {
...
interface UseInquiryBoardPostsReturn {
...
export function useInquiryBoardPosts(
  options: UseInquiryBoardPostsOptions = {},
): UseInquiryBoardPostsReturn {
  ...
  const fetchPosts = useCallback(async (): Promise<BoardPostListResponse | null> => {
    ...
    try {
      ...
      const response = await baasFetch(
        `${BAAS_BASE_URL}/public/boards/${getBaasProjectId()}/${INQUIRY_BOARD_ID}/posts?${params.toString()}`,
        {
          method: 'GET',
      ...
      if (result.result !== 'SUCCESS') {
        throw new Error(result.message || '문의 목록을 불러오지 못했습니다.')
      }

      setData(result.data ?? null)
      return result.data ?? null
    } catch (err) {
      const message = err instanceof Error ? err.message : '문의 목록을 불러오지 못했습니다.'
      setError(message)
      return null
    } finally {
      setIsLoading(false)
