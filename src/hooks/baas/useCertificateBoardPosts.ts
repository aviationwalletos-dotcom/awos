...
async function fetchCertificatePostsPage(offset: number, limit: number): Promise<BoardPostListResponse> {
  ...
  const response = await baasFetch(
    `${BAAS_BASE_URL}/public/boards/${getBaasProjectId()}/${CERTIFICATE_BOARD_ID}/posts?${params.toString()}&author=me`,
    {
      method: 'GET',
...
export function useCertificateBoardPosts(options: UseCertificateBoardPostsOptions = {}): UseCertificateBoardPostsReturn {
  ...
  const refetchAll = useCallback(async (): Promise<BoardPostListItem[] | null> => {
    ...
    try {
      ...
      try {
        const privateRows = await listMyPrivateRecords('user_certificates')
        for (const row of privateRows) {
          collected.push({ id: buildPrivatePostId('user_certificates', row.app_id), title: '' } as unknown as BoardPostListItem)
        }
      } catch {
