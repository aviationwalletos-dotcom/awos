...
//
// 업무기록 데이터 자체는 상세 조회(useBoardPostDetail)로 가져오므로, 이 훅은 "본인 명의로 된
// 업무기록 게시글 id 목록"을 찾는 용도(초기 서버 동기화 등)로 주로 사용한다.
...
interface UseWorkLogBoardPostsReturn {
...
async function fetchWorkLogPostsPage(offset: number, limit: number): Promise<BoardPostListResponse> {
  ...
  if (result.result !== 'SUCCESS' || !result.data) {
    throw new Error(result.message || '업무기록 게시글 목록을 불러오지 못했습니다.')
  }

  return result.data
}

export function useWorkLogBoardPosts(options: UseWorkLogBoardPostsOptions = {}): UseWorkLogBoardPostsReturn {
  ...
  const fetchPosts = useCallback(async (): Promise<BoardPostListResponse | null> => {
    ...
    try {
      const pageData = await fetchWorkLogPostsPage(0, limitRef.current)
      setData(pageData)
      return pageData
    } catch (err) {
      const message = err instanceof Error ? err.message : '업무기록 게시글 목록을 불러오지 못했습니다.'
      setError(message)
      return null
    } finally {
      setIsLoading(false)
  ...
  const refetchAll = useCallback(async (): Promise<BoardPostListItem[] | null> => {
    ...
    try {
      ...
      for (let page = 0; page < MAX_PAGES && offset < totalCount; page += 1) {
        try {
          const pageData = await fetchWorkLogPostsPage(offset, PAGE_SIZE)
          ...
          console.warn('[업무기록 게시글 전체 목록 조회 중 일부 페이지 실패]', err)
          break
        }
      }
