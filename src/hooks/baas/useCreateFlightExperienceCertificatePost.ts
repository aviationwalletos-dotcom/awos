// "비행경력증명서" 게시판 인증 요청 게시글 작성 Hook (요청 1건당 게시글 1건)
...
interface UseCreateFlightExperienceCertificatePostReturn {
  createCertificateRequestPost: (data: BoardPostCreateRequest) => Promise<BoardPostDetail>
  isLoading: boolean
  error: string | null
  reset: () => void
}

export function useCreateFlightExperienceCertificatePost(): UseCreateFlightExperienceCertificatePostReturn {
  const [isLoading, setIsLoading] = useState(false)
  const [error, setError] = useState<string | null>(null)

  const createCertificateRequestPost = useCallback(async (data: BoardPostCreateRequest): Promise<BoardPostDetail> => {
    setIsLoading(true)
    setError(null)
    ...
    try {
      ...
      if (result.result !== 'SUCCESS') {
        throw new Error(result.message || '비행경력증명서 인증 요청 등록에 실패했습니다.')
      }

      return result.data
    } catch (err) {
      const message = err instanceof Error ? err.message : '비행경력증명서 인증 요청 등록에 실패했습니다.'
      setError(message)
  ...
  return { createCertificateRequestPost, isLoading, error, reset }
}
