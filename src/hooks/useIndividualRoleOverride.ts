...
import { ROLE_OVERRIDE_KEY_PREFIX, buildProfileFieldKey, isIndividualRole } from '../lib/profileSettingsSync'
...
function buildStorageKey(accountId: string | null | undefined): string | null {
  ...
  return buildProfileFieldKey(ROLE_OVERRIDE_KEY_PREFIX, accountId)
}

function loadOverride(storageKey: string): IndividualRole | null {
  try {
    const raw = window.localStorage.getItem(storageKey)
    if (!raw) return null
    return isIndividualRole(raw) ? raw : null
  } catch {
    return null
  }
}
...
/**
 * 로그인한 계정(account)별로 분리된 "역할" 로컬 오버라이드를 관리합니다.
 * 계정 가입 시 저장된 individual_role을 변경할 수 있는 서버 API가 없어,
 * 이 브라우저에서 역할을 재설정/변경할 수 있게 하되, "개인설정" 게시판(useProfileSettingsSync)을
 * 통해 서버에도 best-effort로 동기화해 다른 기기에서도 값을 확인/공유할 수 있게 합니다.
 */
export function useIndividualRoleOverride(account: AccountResponse | null | undefined) {
  const accountId = account?.id
  const storageKey = buildStorageKey(accountId)
  const [override, setOverrideState] = useState<IndividualRole | null>(null)
  ...
  useEffect(() => {
    ...
    const serverRole = serverSettings?.individualRoleOverride
    if (!serverRole) return

    try {
      window.localStorage.setItem(storageKey, serverRole)
    } catch {
      // 저장 공간 부족/접근 차단 시 조용히 무시합니다.
    }
    setOverrideState(serverRole)
