...
## 4차 점검
- 서명 스냅샷이 항공기 항목만 담고 있어 **초경량 기록(이륙·착륙 시각, 아워미터, 비행 횟수, 기체, 훈련 내용 등)** 을 바꿔도 못 잡던 것 →
  경량·초경량 전용 항목 전부 포함. 예전 스냅샷(항목 적음)은 공통 항목만 비교해 오판하지 않고, 스냅샷 자체의 해시도 검증(변조 탐지).

## 설명글 정리 (2026-09-06) — 폰에서 회색 설명이 본문보다 많던 것
- 공용 `InfoTip`(ⓘ): 데스크톱 hover, 폰 탭, 키보드 포커스. 본문에 꼭 필요한 문장만 남기고 참고·근거·주의는 ⓘ 뒤로.
- 적용: 승무시간 한도 카드, 새 비행 기록 추가/종이 로그북 가져오기 제목, 엑셀 가져오기 안내(상자 삭제), 가져오기 두 방법 차이(노란 상자 삭제),
  총 비행시간 요약, 자격증 등록, 커런시 현황, 응시경력 진척도, 기록 폼(종류·단독 비행·2인 조종), 계정정보 3개 섹션, 서명 요청함, 교관 승인 신청, 관리자 큐.
- 삭제: "기록은 이 기기와 서버에 함께 안전하게 저장돼요."

## 간소화·가독성 (2026-09-06, 카드덱은 유지)
- 비행기록 필터: `전체 · 미서명 · 필터 ▾`(날짜·기종·종류·월·이월은 ▾ 안). 하위 값은 `↳ 기종` 라벨 + 작은 알약. 날짜는 기록 있는 날짜 드롭다운.
- 목록 버튼: PDF 만 바로, 선택해서 삭제·CSV·전체 삭제는 `⋯` 메뉴(전체 삭제는 맨 아래 빨간 글씨).
- 용어 통일: 탭 "기록 입력", 제목 "새 기록", 버튼 "기록 추가", 제출 "저장", "과거 기록 가져오기". UI 문체 해요체로 일괄 변환(319곳).
- 계정정보: 비밀번호 변경·로그인 방법 연결·생년월일·탈퇴 섹션 접이식(AccountSection). 소속·자격 구분은 펼침.
- (되돌림) 기록 폼 접기는 필수 시간(단발/다발) 누락 우려로 원래대로 전부 표시. 섹션 제목 번호만 제거.
- (되돌림) 승무시간 한도 카드는 모든 계정에 표시(훈련원이 자가용에도 8시간 적용). 카드 배지는 LCL/X-C/NGT/FTD 약어 유지, 초경량 기록엔 배지 없음. 기종 필터는 대소문자·공백 무시해 묶음.
- E2E 텍스트 매처를 문체 변화에 견디게 완화(추가되었습니다 → 추가).
- InfoTip 풍선을 화면 좌표(fixed)로 배치해 폰에서 왼쪽이 잘리던 문제 해결(좌우 8px 안에 들어오게, 아래 공간 없으면 위로).
- 실시간 비행 적합성 카드: 폰에서 2×2(작은 칩), sm 이상은 기존처럼 가로 나열.

## 실시간 비행 적합성 — 등급별 판정 (2026-09-06, 재헌 지적)
- 예전엔 단발·다발 중 하나만 최근 비행경험을 충족해도 "일반 비행 가능"으로 표시. 운항기술기준 8.2.2 는 "동일 등급"이 요건.
- 이제 최근 24개월 안에 탄 등급(육상단발/육상다발/회전익)을 각각 판정. 하나만 되면 배지 **"일부"(주황)**, 전부 되면 "가능", 하나도 안 되면 "제한".
  야간도 같은 방식. 커런시 탭 카드에는 등급별 칩(육상다발 가능 · 육상단발 제한)과 사유가 보임. 요약 카드 칩은 길게 누르면(title) 등급별 내역.
- 단위 테스트 추가(다발 3회·단발 1회 → 일부 + 단발 제한 사유).

## v1.3n 반영 (2026-09-07) — 다른 세션의 UX 정리안 중 채택분
- 3 로그북 헤더에 계정·로그아웃 / 5 적합성 패널의 소속 경고는 기관 공유 켤 때만 / 7 자격증 등록 버튼을 목록 제목 옆(폼 → TS 카드 순서)
- 9 문구: 빈 화면 안내 2곳, 회원가입 제목·버튼, 히어로 CTA, 모바일 메뉴 "도입 문의하기" 삭제 (문의 메일은 그대로)
- Claude 판단으로 추가 채택: 4 폰 탭바 같은 폭 한 줄(아이콘 위·라벨 아래) / 6 "서버와 다시 동기화"를 ⋯ 메뉴로(대기 N건이면 "N건 서버 동기화")
- 미채택: 1 기록 폼 기본 펼침(대표 결정으로 접기 유지) / 2 블록타임 순서 이동 / 8 승무시간 참고치 문구(훈련원 8h 적용 방침)
- 커런시 탭 "최근 비행경험" 헤더 배지: 등급별 판정이 갈리면 "일반 일부 충족 / 야간 일부 충족"(주황) — 적합성 카드와 같은 기준
- 미사용 파일 정리: 저장소에서 직접 지울 20개 → `DELETE-LIST-0907.md`
