...
## 3차 점검에서 잡은 것
- **(중요) 다른 기록에 서명이 붙을 수 있던 경쟁 조건** — 상세 다이얼로그에서 기록을 빠르게 바꾸면 이전 기록의 요청 행이 잠깐 남아
  새 기록에 서명/판정이 적용될 수 있었다. 훅이 id 변경 시 즉시 비우고, 늦게 온 결과는 버리며, 적용 전에 요청 id 를 대조한다.
- 목록 조회 경쟁 조건 — 필터 전환 직후 옛 조회가 늦게 도착하면 옛 목록이 덮어쓰던 것 → 조회 번호로 최신 결과만 반영.
- 교관 반려 사유가 상세 화면을 닫아 두면 사라지던 것 → 기록에 `lastSignatureRejection` 으로 남겨 다음 요청 전까지 표시.
